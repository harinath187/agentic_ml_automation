...
def test_create_run_starts_queued():
    db.create_run("r1", "d1", "uploads/d1.csv", "desc", max_retries=2)
    row = db.get_run("r1")
...
def test_update_run_transitions_through_lifecycle():
    db.create_run("r1", None, "uploads/d1.csv", "desc", 2)
...
def test_list_runs_orders_newest_first_and_respects_limit():
    for i in range(5):
        db.create_run(f"r{i}", None, "uploads/d.csv", "desc", 2)
...
def test_list_runs_filters_by_status():
    ...
    db.create_run("r2", None, "uploads/d.csv", "desc", 2)
...
def test_recover_interrupted_runs_marks_queued_and_running_as_failed():
    db.create_run("r1", None, "uploads/d.csv", "desc", 2)  # stays queued
    db.create_run("r2", None, "uploads/d.csv", "desc", 2)
    db.update_run("r2", status=db.RUNNING, started_at=db.now_iso())
    db.create_run("r3", None, "uploads/d.csv", "desc", 2)
